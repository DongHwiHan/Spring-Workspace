...
public class ChatWebsocketHandler extends TextWebSocketHandler{
	 ...
	 * - 클라이언트와 연결이 종료되면 실행
	 * 
	 * void handlerTransportError(WebSocketSession session, Throwable execption)
	 * - 메세지 전송중 에러발생하면 실행
	 * 
	 * 
	 * */
	...
	//클라이언트와 연결이 수립되고, 통신준비가완료되면 수행되는 메소드
	@Override
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception{
		...
		if(result > 0) {
			...
			for(WebSocketSession s  : sessions ) {
				...
				// 반복을 진행중인 WebSocketSession안에 담겨있는 방번호 == 메세지 안에 담겨있는 방번호 가 일치하는경우 메세지 뿌리기
